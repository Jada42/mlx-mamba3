...
def expand_bc_groups(x: torch.Tensor, nheads: int, dim: int) -> torch.Tensor:
    """Expand B/C group projections to per-head projections."""
    groups = x.shape[dim]
    if groups == nheads:
        return x
    assert nheads % groups == 0, "ngroups must divide nheads"
    return x.repeat_interleave(nheads // groups, dim=dim)
...
def mamba3_siso_scan(
    ...
    for t in range(L):
        ...
        Bx_blended = (1.0 - tr_expanded) * Bx_curr + tr_expanded * 0.5 * (Bx_curr + decay * Bx_prev)
...
def mamba3_mimo_scan(
    ...
    for t in range(L):
        ...
        Bx_blended = (1.0 - tr_e3) * Bx_curr + tr_e3 * 0.5 * (Bx_curr + decay.unsqueeze(-1) * Bx_prev)
...
class Mamba3(nn.Module):
    ...
    def forward(self, u: torch.Tensor) -> torch.Tensor:
        ...
        # Expand from G groups to H heads; each group is shared by H/G heads.
        B_exp = expand_bc_groups(B_normed, self.nheads, dim=3)  # (B, L, R, H, D)
        C_exp = expand_bc_groups(C_normed, self.nheads, dim=3)  # (B, L, R, H, D)
    ...
    def step(
        ...
        B_exp = expand_bc_groups(B_normed, self.nheads, dim=2)  # (B, R, H, D)
        C_exp = expand_bc_groups(C_normed, self.nheads, dim=2)  # (B, R, H, D)
        ...
        if self.is_mimo:
            ...
            Bx_blended = (1.0 - tr_e) * Bx_curr + tr_e * 0.5 * (Bx_curr + decay.unsqueeze(-1) * Bx_prev_state)
            ...
            Bx_blended = (1.0 - tr_e) * Bx_curr + tr_e * 0.5 * (Bx_curr + decay.unsqueeze(-1).unsqueeze(-1) * Bx_prev_state)
